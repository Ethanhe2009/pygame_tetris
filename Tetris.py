...
class Tetris:
    ...
    def new_block(self):
        self.block = Block(3, 0,random.randint(0, len(assets.shapes) - 1))
                           
    def next_block(self):
        self.nextBlock= Block(3,0,random.randint(0, len(assets.shapes) - 1))
    def intersects(self):
        intersection = False
        for i in range(4):
            for j in range(4):
        ...
        for i in range(4):
                for j in range(4):
                    ...
                    if p in self.nextBlock.image():
                        pygame.draw.rect(screen, assets.shapeColors[self.nextBlock.color],(sx + j*30, sy + i*30, 30, 30), 0)
